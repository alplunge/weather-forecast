...
public class BPTrend{
        ...
        try{
            ...
            while(true){
                ...
                if(line == null){  
                    ...
                    String txtDate = wordsArray[0];
                    Date when = format.parse(txtDate);
                    entry.setWhen(when);
                    entry.setPressure(Float.parseFloat(wordsArray[2]));
                    entry.setHumidity(Float.parseFloat(wordsArray[4]));
    ...
    public String doCalc( String from, String to){
        ...
        for ( WeatherEntry x: collectedData){
            if ( (y1==null) && x.getWhen().compareTo(d1) >= 0 ) {
                y1 = x;
                x1 = idx;
            }
            if (x.getWhen().compareTo(d2) >= 0 ) {
                x2 = idx;
                y2 = x; 
                break;
            }
        ...
        if(slope < 0) result += "inclement weather is closing in\n";
        if(slope == 0) result += "current weather conditions are likely to persist\n";
        if(slope > 0) result += "weather conditions are improving\n";
        return result;
    }
